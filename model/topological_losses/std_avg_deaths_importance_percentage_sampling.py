...
def std_avg_deaths_importance_percentage_sampling(model, dataset, labels,
                                                  number_of_points_in_dgm=None, number_of_points_from_ppdd_to_reduce=1,
                                                  sampling_percentage=0.005, alpha=0.5,
                                                  distance_strategy_fn=PearsonDistance.compute_distance_matrix,
                                                  check_validation_same_distance=False):
    """
    Standard deviation of deaths. We maximize it according to experimental results.
    """
    ...
    return alpha * tf.math.reduce_std(deaths_dgm_from_indices) - (1 - alpha) * tf.reduce_mean(deaths_dgm_from_indices)
