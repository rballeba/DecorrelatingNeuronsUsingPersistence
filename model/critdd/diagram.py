...
class Diagram(AbstractDiagram):
    ...
    """
    def __init__(self, X, *, treatment_names=None, maximize_outcome=False, type='wilcoxon'):
        if type not in {'nemenyi', 'wilcoxon'}:
            raise NotImplementedError('The only supported statistical tests are Wilcoxon and Nemenyi')
        ...
        self.type = type
        self.P = stats.pairwise_tests(X, type=self.type)
    ...
    def get_groups(self, alpha=.05, adjustment=None, return_names=False, return_singletons=True):
